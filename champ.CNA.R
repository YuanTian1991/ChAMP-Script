# A script to draw better CNA plot, order Chromosome based on numeric order, instead of character
                      ...
                      arraytype="450K")
{
	...
	if(sampleCNA)
	{
		...
		for(i in 1:ncol(intsqnlogratio))
		{
            ...
            sampleObject[[colnames(intsqnlogratio)[i]]] <- smoothed.CNA.object
	...
	if(groupFreqPlots)
	{
		...
		for(g in 1:length(groups))
		{
            ...
            groupObject[[groups[g]]] <- group.CNA.object
            ...
            {
                o <- order(as.numeric(as.character(ints$CHR)), as.numeric(as.character(ints$MAPINFO)))
                ints = ints[o, ]
